...
def add_reco_Mggb2(df):
  ...
  H2_px = df.dijet_sublead_pt * np.cos(df.dijet_sublead_phi) 
  H2_py = df.dijet_sublead_pt * np.sin(df.dijet_sublead_phi)
  H2_pz = df.dijet_sublead_pt * np.sinh(df.dijet_sublead_eta)
  ...
  df["reco_Mggb2"] = np.sqrt(HH_E**2 - HH_px**2 - HH_py**2 - HH_pz**2)
